...
class ArabicWhyhaffezSectionSixth extends Controller
{
    function whyhaffez_submit(Request $request)
    {
      ...
      $ext = $image->extension();
      $file=time().'.'.$ext;
      $image->storeAs('public/post/arabic_whyhaffezsection6',$file);
      //array
      $data = array(
      ...
      //inserting data
      DB::table('arabic_sixth_whyhaffezs')->insert($data);
    ...
    {
      DB::table('arabic_sixth_whyhaffezs')->where('id',$id)->delete();
      $request->session()->flash('msgForDelete','Data has been Deleted'); 
      return redirect('/admin/arabicadmin/post/whyhaffez/whyhaffezlist');
    }
    function whyhaffez_edit(Request $request , $id)
    {
      $data['sixth_why_result'] = DB::table('arabic_sixth_whyhaffezs')->where('id',$id)->get();
        return view('admin/arabicadmin/post/whyhaffez/sixth_whyhaffez/whyhaffezedit',$data);
    ...
    function whyhaffez_update(Request $request , $id)
    {
      ...
      //updating data
      DB::table('arabic_sixth_whyhaffezs')->where('id',$id)->update($data);
