...
class WhyhaffezSectionFour extends Controller
{
    ...
    function whyhaffez_submit(Request $request)
    {
      ...
      $ext = $image->extension();
      $file=time().'.'.$ext;
      $image->storeAs('public/post/whyhaffezsection4',$file);
      //array
      $data = array(
      ...
      //inserting data
      DB::table('fourth_whyhaffezs')->insert($data);
    ...
    {
      DB::table('fourth_whyhaffezs')->where('id',$id)->delete();
      $request->session()->flash('msgForDelete','Data has been Deleted'); 
      return redirect('/admin/post/whyhaffez/whyhaffezlist');
    }
    function whyhaffez_edit(Request $request , $id)
    {
      $data['fourth_why_result'] = DB::table('fourth_whyhaffezs')->where('id',$id)->get();
        return view('admin/post/whyhaffez/fourth_whyhaffez/whyhaffezedit',$data);
    ...
    function whyhaffez_update(Request $request , $id)
    {
      ...
      //updating data
      DB::table('fourth_whyhaffezs')->where('id',$id)->update($data);
