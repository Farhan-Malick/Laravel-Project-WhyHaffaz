...
class WhyhaffezSectionFive extends Controller
{
    function whyhaffez_submit(Request $request)
    {
      ...
      $ext = $image->extension();
      $file=time().'.'.$ext;
      $image->storeAs('public/post/whyhaffezsection5',$file);
      //array
      $data = array(
      ...
      //inserting data
      DB::table('fifth_whyhaffezs')->insert($data);
    ...
    {
      DB::table('fifth_whyhaffezs')->where('id',$id)->delete();
    ...
    function whyhaffez_update(Request $request , $id)
    {
      ...
      //updating data
      DB::table('fifth_whyhaffezs')->where('id',$id)->update($data);
